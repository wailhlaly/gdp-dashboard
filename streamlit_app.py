...
st.set_page_config(page_title="Saudi Pro Box Strategy", layout="wide", initial_sidebar_state="expanded")
...
TICKERS = {
    # طاقة
    "2222.SR": "أرامكو", "2030.SR": "المصافي", "4200.SR": "الدريس", "5110.SR": "الكهرباء", "4030.SR": "البحري", "2381.SR": "الحفر العربية",
    # مواد أساسية
    "2010.SR": "سابك", "1211.SR": "معادن", "2020.SR": "سابك للمغذيات", "2310.SR": "سبكيم", "2060.SR": "التصنيع",
    "2290.SR": "ينساب", "2350.SR": "كيان", "2380.SR": "رابغ", "2001.SR": "كيمانول", "2170.SR": "اللجين",
    # بنوك
    ...
    # اتصالات
    "7010.SR": "STC", "7020.SR": "موبايلي", "7030.SR": "زين", "7040.SR": "عذيب", "7202.SR": "سلوشنز", "7203.SR": "علم",
    # تجزئة وخدمات
    "4190.SR": "جرير", "4001.SR": "العثيم", "4003.SR": "إكسترا", "4164.SR": "النهدي", "2280.SR": "المراعي",
    "2270.SR": "سدافكو", "1810.SR": "سيرا", "1830.SR": "وقت اللياقة", "4261.SR": "ذيب", "6004.SR": "كاتريون",
    # صحة وتأمين
    "4002.SR": "المواساة", "4013.SR": "سليمان الحبيب", "4004.SR": "دلة", "4007.SR": "الحمادي",
    "8010.SR": "التعاونية", "8210.SR": "بوبا", "8230.SR": "الراجحي تكافل",
    # عقار
    "4300.SR": "دار الأركان", "4250.SR": "جبل عمر", "4220.SR": "إعمار", "4090.SR": "طيبة", "4321.SR": "سينومي سنترز",
    # مؤشر
...
def check_bullish_box(df, atr_series):
    ...
    for i in range(len(prices)):
        ...
        if not in_series:
            if is_green:
                in_series = True; is_bullish = True; start_open = open_p
            elif is_red:
                in_series = True; is_bullish = False; start_open = open_p
        elif in_series:
...
# --- 5. التشغيل ---
st.title("📊 محلل السوق السعودي (النسخة المطورة)")
                        ...
                        if col in df.columns:
                            ...
                            if len(df) > 60:
                                ...
                                if found_boxes:
                                    ...
                                    # الشرط الجديد: هل السعر الحالي داخل الصندوق؟
                                    if box_bottom <= current_price <= box_top:
                                        
                                        # حساب المنتصف والمسافة
                                        midpoint = (box_top + box_bottom) / 2
                                        
                                        # تحديد الحالة (إشارة بصرية)
                                        if current_price >= midpoint:
                                            status = "أعلى المنتصف 🟢"
                                        else:
                                            status = "أدنى المنتصف 🔴"
                                            
                                        # نسبة البعد عن المنتصف
                                        ...
                                        st.session_state['boxes'].append({
                                            ...
                                            "قمة الصندوق": box_top,
                                            "قاع الصندوق": box_bottom,
                                            "المنتصف": midpoint,
                                            "الإشارة": status,
                                            "البعد عن المنتصف %": dist_pct,
                                            "منذ": latest_box['Days_Ago']
                                        })
                                ...
                                if len(tail) == 4:
                                    ...
                                    if rsi_break and ema_break:
                                        ...
                                        st.session_state['signals'].append({"الاسم": name, "السعر": current_price, "RSI": last_row['RSI'], "MACD": macd_status})
                    except: continue
        except: pass
...
if st.session_state['data']:
    ...
    # --- TAB 1: الصناديق (المطور) ---
    with t1:
        if st.session_state['boxes']:
            st.markdown("### أسهم تتداول حالياً **داخل** الصندوق الصعودي")
            df_boxes = pd.DataFrame(st.session_state['boxes'])
            
            # ترتيب حسب البعد عن المنتصف (الأقرب للارتداد من القاع أو اختراق المنتصف)
            df_boxes = df_boxes.sort_values(by="البعد عن المنتصف %", ascending=True)
            
            # دالة تلوين الإشارة
            def color_signal(val):
                color = '#4caf50' if "أعلى" in val else '#ff5252'
                return f'color: {color}; font-weight: bold;'
            
            st.dataframe(
                df_boxes.style.format({
                    "السعر الحالي": "{:.2f}", "قمة الصندوق": "{:.2f}", 
                    "قاع الصندوق": "{:.2f}", "المنتصف": "{:.2f}", 
                    "البعد عن المنتصف %": "{:.2f}%"
                })
                .map(color_signal, subset=['الإشارة'])
                .background_gradient(cmap='Greys', subset=['منذ']),
                use_container_width=True
            )
        else:
            st.info("لا توجد أسهم تتداول داخل نطاق صندوق صعودي حالياً.")
    ...
    with t4:
        ...
        if sel:
            ...
            if box_res:
                ...
                # المستطيل
                fig.add_shape(type="rect", x0=df_chart.index[-latest['Days_Ago']-2], x1=df_chart.index[-1], 
                              y0=latest['Box_Bottom'], y1=latest['Box_Top'], 
                              line=dict(color="green", width=1, dash="dot"), fillcolor="rgba(0,255,0,0.05)", row=1, col=1)
                
                # خط المنتصف
                fig.add_shape(type="line", x0=df_chart.index[-latest['Days_Ago']-2], x1=df_chart.index[-1],
                              y0=midpoint, y1=midpoint,
