...
TICKERS = {
    ...
    "7202.SR": "سلوشنز", "7203.SR": "علم", "7200.SR": "المعمر (MIS)", "7201.SR": "بحر العرب", 
    "7204.SR": "توبي",
...
def check_bullish_box(df, atr_series):
    in_series = False; is_bullish = False
    start_open = 0.0; end_close = 0.0; found_boxes = []
    ...
    for i in range(len(prices)):
        ...
        if not in_series:
            ...
            if is_bullish and is_green: end_close = close
                ...
                if price_move >= threshold and is_bullish:
...
# --- 5. التشغيل ---
st.title("📊 محلل السوق السعودي (النسخة النهائية)")
...
if st.button("🚀 تشغيل المسح"):
    ...
    for i in range(0, total_tickers, chunk_size):
        ...
        status_text.text(f"جاري التحليل... الدفعة {i//chunk_size + 1}")
                    ...
                    try:
                        ...
                        if col in df.columns:
                            ...
                            if len(df) > 60:
                                ...
                                # إنشاء رابط TradingView
                                ticker_clean = symbol.replace('.SR', '')
                                tv_link = f"https://www.tradingview.com/chart/?symbol=TADAWUL:{ticker_clean}"
                                
                                st.session_state['history'][name] = df
                                st.session_state['data'].append({
                                    "Name": name, "Symbol": symbol, "Price": current_price,
                                    "Change": last_row['Change'], "RSI": last_row['RSI'], "MACD": last_row['MACD'],
                                    "TV_Url": tv_link # الرابط هنا
                                })
                                ...
                                if found_boxes:
                                    ...
                                    if box_bottom <= current_price <= box_top:
                                        midpoint = (box_top + box_bottom) / 2
                                        status = "أعلى المنتصف 🟢" if current_price >= midpoint else "أدنى المنتصف 🔴"
                                        dist_pct = ((current_price - midpoint) / midpoint) * 100
                                        st.session_state['boxes'].append({
                                            "الاسم": name, "السعر الحالي": current_price, "قمة الصندوق": box_top, "قاع الصندوق": box_bottom,
                                            "المنتصف": midpoint, "الإشارة": status, "البعد عن المنتصف %": dist_pct, "منذ": latest_box['Days_Ago'],
                                            "TV_Url": tv_link # الرابط هنا أيضاً
                                        })

                                # القناص
                                tail = df.tail(4)
                                if len(tail) == 4:
                                    rsi_break = False; ema_break = False
                                    ...
                                    if rsi_break and ema_break:
                                        ...
                                        st.session_state['signals'].append({"الاسم": name, "السعر": current_price, "RSI": last_row['RSI'], "MACD": macd_status, "TV_Url": tv_link})
                    except: continue
        except: pass
...
if st.session_state['data']:
    ...
    k1.metric("الشركات", len(df_all))
    k2.metric("القناص", len(st.session_state['signals']))
    k3.metric("داخل الصندوق 📦", len(st.session_state['boxes']))
    k4.metric("السوق أخضر", len(df_all[df_all['Change'] > 0]))
    
    st.markdown("---")
    t1, t2, t3, t4 = st.tabs(["📦 استراتيجية الصندوق", "🎯 إشارات القناص", "📋 السوق الشامل", "📈 الشارت"])
    
    # تعريف إعدادات عمود الرابط لاستخدامه في كل الجداول
    link_config = st.column_config.LinkColumn("TV", display_text="📈 شارت", help="اضغط لفتح TradingView")

    with t1:
        if st.session_state['boxes']:
            st.markdown("### أسهم تتداول داخل الصندوق")
            df_boxes = pd.DataFrame(st.session_state['boxes']).sort_values(by="البعد عن المنتصف %", ascending=True)
            def color_signal(val): return f'color: {"#4caf50" if "أعلى" in val else "#ff5252"}; font-weight: bold;'
            
            # ترتيب الأعمدة لوضع الرابط في النهاية
            cols = ["الاسم", "السعر الحالي", "الإشارة", "المنتصف", "البعد عن المنتصف %", "TV_Url"]
            
            st.dataframe(
                df_boxes[cols].style.format({"السعر الحالي": "{:.2f}", "المنتصف": "{:.2f}", "البعد عن المنتصف %": "{:.2f}%"})
                .map(color_signal, subset=['الإشارة']),
                column_config={"TV_Url": link_config}, # تفعيل الرابط
                use_container_width=True
            )
        else: st.info("لا توجد صناديق نشطة.")

    with t2:
        if st.session_state['signals']:
            df_sig = pd.DataFrame(st.session_state['signals'])
            st.dataframe(df_sig, column_config={"TV_Url": link_config}, use_container_width=True)
    ...
    with t3:
        ...
        cols_to_show = ["الاسم", "السعر", "التغير %", f"RSI ({RSI_PERIOD})", "TV_Url"]
        st.dataframe(
    ...
    with t4:
        ...
        if sel:
            ...
            if box_res:
                ...
                fig.add_shape(type="rect", x0=df_chart.index[-latest['Days_Ago']-2], x1=df_chart.index[-1], y0=latest['Box_Bottom'], y1=latest['Box_Top'], line=dict(color="green", width=1, dash="dot"), fillcolor="rgba(0,255,0,0.05)", row=1, col=1)
                fig.add_shape(type="line", x0=df_chart.index[-latest['Days_Ago']-2], x1=df_chart.index[-1], y0=midpoint, y1=midpoint, line=dict(color="blue", width=2), row=1, col=1)
